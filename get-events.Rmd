...
get_event_data_frame <- function(group_id) {
  ...
  while (offset >= 0) {
    ...
    tbl_json <- get_json_tbl(offset = offset, page = 200, group_id = group_id)
  	 ...
  	 event_strings <- tbl_json %>% gather_array %>% 
  	   ...
  	   # mutate(eventber_of_group = group_id)
  
  	 event_numerics <- tbl_json %>% gather_array %>% 
  	   my_spread_values(
  	      num_list, jnumber ) %>% 
  	    select(-document.id, rec_num = array.index) 
  	 event_numerics$created <- as.POSIXct(event_numerics$created/1000, origin = "1970-01-01")
  	 event_numerics$time <- as.POSIXct(event_numerics$time/1000, origin = "1970-01-01")
  	 event_numerics$updated <- as.POSIXct(event_numerics$updated/1000, origin = "1970-01-01")
  	 event_numerics$duration_minutes <- event_numerics$duration/60000
  ...
  #     if (page == 0 ) {
  #       group_id_events <- events
  # 	    group_id_topics <- member_topics
  # 	  } else {
  #       group_id_events <- bind_rows(group_id_events, events)
  ...
  #     }
    ...
    cat("+")
    return(list("events" = group_id_events))
...
selected_events  %>% filter(tag == "R Users" &  yes_rsvp_count < 5)  %>% select( time, name)
...
ggplot(org_events, aes(x = time, y = yes_rsvp_count, colour = tag) ) + geom_smooth(se = F) +
  geom_point() +
  ggtitle("Event size, selected meetups") +
  xlab("Date") +
  ylab("Interest measured by 'yes' RSVP's")
